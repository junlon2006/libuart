...
typedef struct {
  ...
  pthread_mutex_t
      app_send_sync_lock; /* avoid app send concurrency, out of sequence */
  uni_bool acked;
...
{
  uint16_t checksum =
      crc16((const char *) packet->payload_len, sizeof(CommPayloadLen));
...
static void _payload_set(CommProtocolPacket *packet, char *buf,
                         CommPayloadLen len)
{
  if (NULL != buf && 0 < len) {
...
{
  CommChecksum checksum = _byte2_big_endian_2_u16(
      packet->checksum);  /* get the checksum from packet */
  _checksum_calc(packet); /* calc checksum again */
  int valid = (checksum ==
               _byte2_big_endian_2_u16(
                   packet->checksum)); /* check whether checksum valid or not */
  if (!valid) {
...
{
  CommProtocolPacket *packet = (CommProtocolPacket *) uni_malloc(
...
static void _assmeble_packet(CommProtocolPacket *packet, CommCmd cmd,
                             char *payload, CommPayloadLen payload_len,
                             uni_bool reliable, CommSequence seq,
                             uni_bool is_ack_packet, uni_bool is_nack_packet)
{
  _sync_set(packet);
...
static int _assemble_and_send_frame(CommCmd cmd, char *payload,
                                    CommPayloadLen payload_len,
                                    CommAttribute *attribute, CommSequence seq,
                                    uni_bool is_ack_packet,
                                    uni_bool is_nack_packet)
{
  int ret = 0;
  if (_is_protocol_buffer_overflow(sizeof(CommProtocolPacket) + payload_len)) {
  ...
  _assmeble_packet(packet, cmd, payload, payload_len,
                   attribute && attribute->reliable, seq, is_ack_packet,
                   is_nack_packet);
                                      ...
                                      CommAttribute *attribute)
{
  ...
  ret = _assemble_and_send_frame(cmd, payload, payload_len, attribute, 0, false,
                                 false);
                                     ...
                                     CommPayloadLen *orginal_len)
{
  CommPayloadLen new_size =
      *orginal_len * 2 + sizeof(struct header); /* cover header */
...
static void _reset_protocol_buffer_status(unsigned int *index,
                                          CommPayloadLen *length, uint16_t *crc)
{
  *index = 0;
  *length = 0;
  *crc = 0;
