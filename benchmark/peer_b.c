...
static void _recv_comm_packet(CommPacket *packet)
{
  ...
  if (now - start > 1000) {
    ...
    printf(
        "[%d:ER%d] total=%ldKB, cost=%ld-%02ld:%02ld:%02ld, speed=%.2fKB/s, BW "
        "RATIO=%.2f%%\n",
        BAUD_RATE, err_cnt, (long) total_len >> 10, (long) cost / (3600 * 24),
        (long) cost % (3600 * 24) / 3600, (long) cost % (3600 * 24) % 3600 / 60,
        (long) cost % (3600 * 24) % 3600 % 60, avg_speed,
        avg_speed / (BAUD_RATE >> 13) * 100);
    start = now;
  }
}
...
int main()
{
  ...
  while (1) {
    user_data.seq++;
    CommProtocolPacketAssembleAndSend(1, (char *) &user_data, sizeof(user_data),
                                      &attr);
  }
